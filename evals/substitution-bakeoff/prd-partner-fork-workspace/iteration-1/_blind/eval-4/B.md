...
**Skill:** `[SKILL]` · **Mode:** PRD Mode → **Dev-Team** output
...
The request — *"Add a feature where we auto-summarise the user's uploaded documents with an LLM and surface the summary in the digest"* — is one sentence, and by `[SKILL]`'s own mode-detection rules it is **fuzzy**, which routes to Discovery Mode. Discovery Mode is defined as a live `grilling` session: one question at a time, waiting for each answer. That is impossible unattended.
...
`[SKILL]` says to push back when a problem is stated as a solution. This one is — twice over. It names a mechanism ("summarise with an LLM") and a surface ("in the digest") without naming what is broken. Three different problems produce this same sentence, and they do **not** converge on the same v1:
...
## 16. Skill improvement notes (`[SKILL]` closing hook)
