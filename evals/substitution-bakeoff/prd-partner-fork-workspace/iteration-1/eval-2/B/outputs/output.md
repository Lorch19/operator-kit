Running **`prd-partner`** — PRD Mode, **Dev-Team** output. Audience cue is "engineering needs it"; "assume we've decided to build it" closes the build/don't-build question, so no discovery detour. I've taken the build decision as made and noted the one residual risk at the end rather than re-opening it here.
