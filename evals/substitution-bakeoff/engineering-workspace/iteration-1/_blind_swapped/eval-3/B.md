...
**Skill running: `[SKILL]`** (primary — a flake is a bug with a
non-deterministic trigger, which is the exact case that skill's Phase 1 is built for),
with **`[SKILL]`** for Phase 5 (the regression test).
...
> The `[SKILL]` skill's Phase 1 gate: name **one command you have already run**,
...
GNU/BSD `od`; expect ten minutes of fitting. Most importantly, the `[SKILL]` Phase 1
