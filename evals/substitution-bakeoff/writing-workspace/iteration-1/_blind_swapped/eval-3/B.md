...
4. **One repo or everywhere?** → *Assumed one repo — you said "migrations," which implies a specific codebase. Put this in that repo's `[PROJECT_RULES]`, not the global one. If it turns out to be three repos, the rule text is identical; only the file it lives in changes.*
...
2. Paste the block into the repo's `[PROJECT_RULES]`, fill DB-001's precedent slots, drop your stack's round-trip command into the self-check.
