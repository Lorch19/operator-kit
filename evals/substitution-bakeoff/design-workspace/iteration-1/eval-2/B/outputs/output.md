...
**Skills run:** `design-tools/ux-copy` (primary), `design-tools/design-critique` (diagnosis), `design-tools/accessibility-review` (audit of the proposed design), `design-tools/design-handoff` (engineering spec).
...
*(skill: design-critique. The artifact under review is the described current state: a projects page rendering the string "No projects".)*
...
| Noun for the action | Header button may say "New project" while docs/onboarding say "Create a project". | Pick one verb and use it everywhere — recommendation: **Create project**. `ux-copy` principle 3 (Consistent). |
...
*(skill: ux-copy. Pattern applied: "Empty States — what this is + why it's empty + how to start.")*
...
- **"Create your first project" as the heading, not "No projects yet."** The heading is the one line everyone reads. Spending it on a fact the user can already see (the list is empty) wastes it. Spend it on the instruction. Matches `ux-copy` CTA principle — start with a verb.
...
*(skill: accessibility-review. Standard: WCAG 2.1 AA. Audited against the proposed design in §2 plus the handoff spec in §4, since no live page exists. Every finding below is a build instruction, not a retro-fix.)*
...
*(skill: design-handoff. Written to be implementable without a follow-up conversation. Token values are placeholders — swap for your real scale, keep the semantic names.)*
