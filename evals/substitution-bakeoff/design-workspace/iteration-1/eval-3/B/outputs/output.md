...
**Skill:** `ux-reviewer` (primary) · `ux-creator` Phase 2 for the DESIGN-TARGETS.md draft in Part 3.
...
**Confidence: LOW-MEDIUM, and the reason matters.** Pass 0 of `ux-reviewer` requires a codebase
scan and, if a build exists, rendered frames before code. I have neither: no spec, no mocks, no
routes, no screenshots, no `docs/design/DESIGN-TARGETS.md`. Per the skill's own rule I am saying so
...
| Q2 | Does any written success criteria / design-targets doc exist for this redesign? | No | If one exists, replace Part 3 with it and score against it instead. |
...
# Part 3 — Provisional DESIGN-TARGETS.md

Paste into `docs/design/DESIGN-TARGETS.md`, correct the guesses, get it approved Thursday. **This is
...
```markdown
# Design Targets: Settings Redesign
...
nothing above finds it. The only fix is running `ux-reviewer` properly against the actual file, in a
