...
**Skill:** `[SKILL]` (five passes; Pass 5 floor checks carry most of the weight here)
**Against:** `[TARGETS_DOC]` — **PROVISIONAL**. None exists; I drafted the ten criteria
...
- **No locked visual direction exists.** There is no `[TARGETS_DOC]`, so no criterion names
...
theme is out of scope for this release, say so as a **deliberate deferral in the [TARGETS_DOC]**
...
*Countable artifact:* `docs/design/[TARGETS_DOC]` — does not exist.
...
`docs/design/[TARGETS_DOC]` along with **one named visual direction** in your product's own
