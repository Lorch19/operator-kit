...
**Skills run:** `design-tools/design-critique` (primary), `design-tools/ux-copy` (secondary, for the copy rewrites in §6).
**Skills deliberately not run:** `accessibility-review`, `design-handoff` — see §9 for why.
...
- **`design-handoff` — not run.** Handoff specs are for a design that's settled and heading to engineering. Yours is under diagnosis and I'm recommending you delete a screen. Writing tokens, props, and breakpoint tables for a flow that should lose a third of itself would be work you'd throw away. Run `design-handoff` after you've picked from §4 and settled the new flow.
